...
def gen_golden_data_simple():
    m = 64
    n = 32
    input_xyz = np.random.uniform(-10, 10, [n, 3]).astype(np.float32)
    center_xyz = np.random.uniform(-10, 10, [m, 3]).astype(np.float32)
    xyz_batch_cnt = np.array([16,16]).astype(np.int32)
    center_xyz_batch_cnt = np.array([32,32]).astype(np.int32)
    min_radius = 10
    max_radius = 50
    sample_num = 13
