...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR gamma, GM_ADDR beta, GM_ADDR y, GM_ADDR mean, GM_ADDR rstd, int32_t chunk_size, int32_t batch_size, int32_t num_groups, int32_t num_channels, int32_t total_size, float epsilon) {
        ...
        for (int index = 0, i = 0; i < batch_size; ++i) {
            for (int j = 0; j < num_groups; ++j) {
                ...
                for (int x = 0; x < chunk_size; ++x) {
                    ...
                    rstd[i * num_groups + j] += sum / length;
                }
            }
        }
...
private:
    ...
    int32_t chunk_size, batch_size, num_groups, num_channels, total_size;
    float epsilon;
    float mean[1024 + 512];
    float rstd[1024 + 512];
};
extern "C" __global__ __aicore__ void group_norm_v2(GM_ADDR x, GM_ADDR gamma, GM_ADDR beta, GM_ADDR y, GM_ADDR mean, GM_ADDR rstd, GM_ADDR workspace, GM_ADDR tiling) {
    ...
    op.Init(x, gamma, beta, y, mean, rstd, tiling_data.chunk_size, tiling_data.batch_size, tiling_data.num_groups, tiling_data.num_channels, tiling_data.total_size, tiling_data.epsilon);
    op.Process();
}
